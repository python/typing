...
from typing_extensions import NoReturn, ClassVar, Final, IntVar, Literal, Type, NewType, TypedDict, Annotated, get_type_hints
try:
...
class AnnotatedTests(BaseTestCase):
    ...
    def test_specialize(self):
        ...
        if PEP_560:
            self.assertEqual(L[int].__type__, List[int])
        with self.assertRaises(TypeError):
            LI[int]
        with self.assertRaises(TypeError):
            L[int, float]
    ...
    def test_cannot_instantiate_forward(self):
        ...
        with self.assertRaisesRegex(
            TypeError,
            "Cannot create an instance .* forward reference"
        ):
            A(5)
    ...
    def test_cannot_instantiate_type_var(self):
        ...
        with self.assertRaisesRegex(
            TypeError,
            "Cannot create an instance .* type variable"
        ):
            A(5)

    def test_cannot_getattr_typevar(self):
        with self.assertRaisesRegex(TypeError, "Cannot access .* type variable"):
            Annotated[T, (5, 7)].x
    ...
    def test_cannot_subclass(self):
        with self.assertRaisesRegex(TypeError, "Cannot inherit from Annotated"):
            class C(Annotated):
                pass

        with self.assertRaisesRegex(TypeError, "Cannot inherit from Annotated"):
            class C(Annotated[int, (5, 6)]):
                pass
