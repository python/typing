...
Do not annotate ``self`` and ``cls`` in method definitions unless an explicit
annotation is needed to constrain or relax the receiver type.
...
No::
    ...
    class Foo:
        def bar1(self: Self) -> None: ...
        def bar2(self: Foo) -> None: ...
        @classmethod
